...
class RoutingManager(object):
    ...
    maintenance_delay = property(get_maintenance_delay)
    ...
    def do_maintenance(self):
        ...
        # and the next if is executed
        if self.maintenance_mode == NORMAL_MODE:
            for _ in range(len(self.maintenance_tasks)):
                ...
                # No queries to be sent right now
                return
        return [self._get_maintenance_query(node_)]
    ...
    def _ping_a_staled_rnode(self):
        starting_index = self.next_maintenance_index
        result = None
        while not result:
            # Find a non-empty bucket
            sbucket = self.table.get_sbucket(self.next_maintenance_index)
            m_bucket = sbucket.main
            self.next_maintenance_index = (
                self.next_maintenance_index + 1) % (NUM_BUCKETS - 1)
            if m_bucket:
                rnode = m_bucket.get_stalest_rnode()
                if time.time() > rnode.last_seen + QUARANTINE_PERIOD:
                    result = rnode
            if self.next_maintenance_index == starting_index:
...
#                self.my_node.log_distance(node_))
    ...
    def _get_maintenance_query(self, node_):
        return querier.Query(self.find_closest_msg, node_)


    
        if node_.id:
            log_distance = self.table.find_next_bucket_with_room_index(
                node_)
        else:
            # Bootstrap nodes don't have id
            return self.find_closest_msg
        if log_distance:
            target = self.my_node.id.generate_close_id(log_distance)
            return message.OutgoingFindNodeQuery(self.my_node.id, target)
        else:
            return self.ping_msg
...
#            print 'on_query: queued %d' % (
#                self.my_node.log_distance(ndoe_))
    ...
    def on_response_received(self, node_, rtt, nodes):
        ...
        '''
        highest_rtt_rnode = m_bucket.get_highest_rtt_rnode(4)
...
#               node_.log_distance(self.my_node))
            ...
            rnode = node_.get_rnode()
            m_bucket.add(rnode)
# No need to update and modify num_rnodes       
#            self.table.update_lowest_index(log_distance)
#            self.table.num_rnodes += 1
            self._update_rnode_on_response_received(rnode, rtt)
            return
...
class _QueryReceivedQueue(object):
    ...
    def pop(self, pos):
        result = None
        while self._queue and not result:
