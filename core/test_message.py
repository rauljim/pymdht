...
class TestEvilIncomingQueries: #aka invalid bencode messages
    ...
    def _get_queries(self): 
        return [m.OutgoingPingQuery(tc.CLIENT_ID),
    ...
    def _get_responses(self):
        return [m.OutgoingPingResponse(tc.SERVER_ID),
                m.OutgoingFindNodeResponse(tc.SERVER_ID,
                                           tc.NODES),
                m.OutgoingGetPeersResponse(tc.SERVER_ID,
                                           tc.TOKEN,
...
class TestIncomingMsg:
    ...
    def setup(self):
        b_ping = m.OutgoingPingQuery(tc.CLIENT_ID).stamp(
    ...
    def test_unknown_error(self):
        ...
        b_err = m.OutgoingErrorMsg(error_code).stamp(tc.TID, tc.SERVER_NODE)
...
b_ping_q = m.OutgoingPingQuery(tc.CLIENT_ID).stamp(tc.TID, tc.SERVER_NODE) 
b_fn_q = m.OutgoingFindNodeQuery(
    tc.CLIENT_ID, tc.NODE_ID, None).stamp(tc.TID, tc.SERVER_NODE)
b_gp_q = m.OutgoingGetPeersQuery(tc.CLIENT_ID, tc.INFO_HASH, None).stamp(
    tc.TID, tc.SERVER_NODE)
b_ap_q = m.OutgoingAnnouncePeerQuery(
...
b_ping_r = m.OutgoingPingResponse(tc.CLIENT_ID).stamp(tc.TID, tc.SERVER_NODE)
b_fn2_r = m.OutgoingFindNodeResponse(tc.CLIENT_ID, tc.NODES).stamp(tc.TID, tc.SERVER_NODE)
b_gp_r = m.OutgoingGetPeersResponse(tc.CLIENT_ID, tc.TOKEN, tc.NODES,
                                    peers=tc.PEERS).stamp(tc.TID, tc.SERVER_NODE)
b_ap_r = m.OutgoingAnnouncePeerResponse(tc.CLIENT_ID).stamp(tc.TID, tc.SERVER_NODE)
...
class TestSanitizeErrorError:
    ...
    def test(self):
        msg_out = m.OutgoingErrorMsg(1).stamp(tc.TID, tc.SERVER_NODE)
        assert_raises(m.MsgError, m.IncomingMsg,
                      Datagram(msg_out, tc.CLIENT_ADDR))
        # Unknown error doesn't raise m.MsgError
        msg_out = m.OutgoingErrorMsg((1,1)).stamp(tc.TID, tc.SERVER_NODE)
